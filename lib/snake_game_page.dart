...
class _SnakeGamePageState extends State<SnakeGamePage> {
  ...
  late int foodPoistiona;
  late int foodPoistionb;
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: const Text("Tu serpiente chocó!"),
          actions: [
            TextButton(
                onPressed: () {
  ...
  void generateFood() {//funcion para la comida aparece
  ...
  void generateFooda() {//funcion para la comida aparece
    foodPoistiona = Random().nextInt(row * column);
    if (borderList.contains(foodPoistiona)) {
      generateFooda();
  ...
  Color fillBoxColor(int index) {
    if (borderList.contains(index))
      return Color.fromARGB(255, 17, 2, 124);
    else {
      if (snakePosition.contains(index)) {
        if (snakeHead == index) {
          return Color.fromARGB(255, 70, 3, 101);
        } else {
          return Color.fromARGB(255, 148, 103, 103).withOpacity(0.9);
        }
      } else {
    ...
    return Color.fromARGB(255, 236, 137, 202).withOpacity(0.05);
  }
...
Color getRandomColor() {
  ...
  return Color.fromARGB(
    255, // Opacidad máxima
    random.nextInt(256), // Valor rojo
    random.nextInt(256), // Valor verde
    random.nextInt(256), // Valor azul
  );
}
  makeBorder() {
